...
export default { async fetch(req: Request) {
  ...
  try {
    ...
    if(input.action==='connect') {
      ...
      await table('sync_runs',{method:'POST',body:JSON.stringify({marketplace_account_id:id,user_id:user.id,status:'completed',records_processed:1,finished_at:new Date().toISOString()})}); return json({ok:true,accountId:id,accountName:info.name,settings:info.settings});
    }
